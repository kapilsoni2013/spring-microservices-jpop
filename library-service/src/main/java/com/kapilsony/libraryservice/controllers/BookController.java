...
public class BookController {
    ...
    public ResponseEntity<BookResponse> getBook(@PathVariable("book_id") Long book_id) {
        return ResponseEntity.status(HttpStatus.OK).body(bookService.findBookById(book_id));
    }

    @PostMapping("/books")
    public ResponseEntity<Void> addBookToLibrary(@RequestBody BookRequest bookRequest) {
        bookService.addBookToLibrary(bookRequest);
        return new ResponseEntity<>(HttpStatus.CREATED);
    }

    @PutMapping("/books/{book_id}")
    public ResponseEntity<Void> updateBookInLibrary(@PathVariable("book_id") Long book_id,@RequestBody BookRequest bookRequest) {
        bookService.updateBookInLibrary(book_id,bookRequest);
        return new ResponseEntity<>(HttpStatus.OK);
    }

    @DeleteMapping("/books/{book_id}")
    public ResponseEntity<Void> deleteBookFromLibrary(@PathVariable("book_id") Long book_id) {
